.class public Lcom/samsung/android/app/SemExecutableWhitelist;
.super Ljava/lang/Object;
.source "SemExecutableWhitelist.java"


# static fields
.field private static final blacklist DEBUG:Z

.field private static final blacklist SIGNATURE_MAX:I = 0x3

.field private static final blacklist SIGNATURE_SAMSUNG_APPLICATION:I = 0x2

.field private static final blacklist SIGNATURE_SAMSUNG_PLATFORM:I = 0x1

.field private static final blacklist SIGNATURE_SAMSUNG_SHARED:I = 0x4

.field private static final blacklist TAG:Ljava/lang/String; = "SemExecutableWhitelist"

.field private static blacklist sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;


# instance fields
.field private blacklist mSignaturesMap:Landroid/util/SparseArray;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Landroid/util/SparseArray<",
            "Landroid/content/pm/Signature;",
            ">;"
        }
    .end annotation
.end field

.field private blacklist mWhiteList:Ljava/util/HashMap;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/HashMap<",
            "Ljava/lang/String;",
            "Ljava/lang/Integer;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method static constructor blacklist <clinit>()V
    .registers 1

    .line 30
    invoke-static {}, Landroid/os/Debug;->semIsProductDev()Z

    move-result v0

    sput-boolean v0, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    .line 34
    const/4 v0, 0x0

    sput-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    return-void
.end method

.method public constructor blacklist <init>()V
    .registers 2

    .line 22
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    .line 32
    new-instance v0, Landroid/util/SparseArray;

    invoke-direct {v0}, Landroid/util/SparseArray;-><init>()V

    iput-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    .line 33
    new-instance v0, Ljava/util/HashMap;

    invoke-direct {v0}, Ljava/util/HashMap;-><init>()V

    iput-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mWhiteList:Ljava/util/HashMap;

    return-void
.end method

.method private blacklist SemExecutableWhitelist()V
    .registers 1

    .line 44
    invoke-direct {p0}, Lcom/samsung/android/app/SemExecutableWhitelist;->setupSignaturesMap()V

    .line 45
    invoke-direct {p0}, Lcom/samsung/android/app/SemExecutableWhitelist;->setupWhiteList()V

    .line 46
    return-void
.end method

.method public static blacklist getInstance()Lcom/samsung/android/app/SemExecutableWhitelist;
    .registers 1

    .line 37
    sget-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    if-nez v0, :cond_b

    .line 38
    new-instance v0, Lcom/samsung/android/app/SemExecutableWhitelist;

    invoke-direct {v0}, Lcom/samsung/android/app/SemExecutableWhitelist;-><init>()V

    sput-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    .line 40
    :cond_b
    sget-object v0, Lcom/samsung/android/app/SemExecutableWhitelist;->sInstance:Lcom/samsung/android/app/SemExecutableWhitelist;

    return-object v0
.end method

.method private blacklist getSignatures(I)Ljava/util/ArrayList;
    .registers 6
    .param p1, "type"    # I
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(I)",
            "Ljava/util/ArrayList<",
            "Landroid/content/pm/Signature;",
            ">;"
        }
    .end annotation

    .line 125
    new-instance v0, Ljava/util/ArrayList;

    invoke-direct {v0}, Ljava/util/ArrayList;-><init>()V

    .line 126
    .local v0, "list":Ljava/util/ArrayList;, "Ljava/util/ArrayList<Landroid/content/pm/Signature;>;"
    const/4 v1, 0x0

    .local v1, "i":I
    :goto_6
    const/4 v2, 0x3

    if-ge v1, v2, :cond_1d

    .line 127
    const/4 v2, 0x1

    shl-int/2addr v2, v1

    .line 128
    .local v2, "flag":I
    and-int v3, p1, v2

    if-eqz v3, :cond_1a

    .line 129
    iget-object v3, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    invoke-virtual {v3, v2}, Landroid/util/SparseArray;->get(I)Ljava/lang/Object;

    move-result-object v3

    check-cast v3, Landroid/content/pm/Signature;

    invoke-virtual {v0, v3}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    .line 126
    .end local v2    # "flag":I
    :cond_1a
    add-int/lit8 v1, v1, 0x1

    goto :goto_6

    .line 132
    .end local v1    # "i":I
    :cond_1d
    return-object v0
.end method

.method private blacklist hasSamsungKey([Landroid/content/pm/Signature;)Z
    .registers 3
    .param p1, "signatures"    # [Landroid/content/pm/Signature;

    .line 120
    const/4 v0, 0x7

    invoke-direct {p0, p1, v0}, Lcom/samsung/android/app/SemExecutableWhitelist;->isMatchedSignature([Landroid/content/pm/Signature;I)Z

    move-result v0

    return v0
.end method

.method private blacklist isMatchedSignature([Landroid/content/pm/Signature;I)Z
    .registers 11
    .param p1, "signatures"    # [Landroid/content/pm/Signature;
    .param p2, "type"    # I

    .line 136
    invoke-direct {p0, p2}, Lcom/samsung/android/app/SemExecutableWhitelist;->getSignatures(I)Ljava/util/ArrayList;

    move-result-object v0

    .line 137
    .local v0, "SIGNATURES":Ljava/util/ArrayList;, "Ljava/util/ArrayList<Landroid/content/pm/Signature;>;"
    const/4 v1, 0x0

    if-eqz p1, :cond_2d

    if-eqz v0, :cond_2d

    .line 138
    array-length v2, p1

    move v3, v1

    :goto_b
    if-ge v3, v2, :cond_2d

    aget-object v4, p1, v3

    .line 139
    .local v4, "signature":Landroid/content/pm/Signature;
    invoke-virtual {v0}, Ljava/util/ArrayList;->iterator()Ljava/util/Iterator;

    move-result-object v5

    :goto_13
    invoke-interface {v5}, Ljava/util/Iterator;->hasNext()Z

    move-result v6

    if-eqz v6, :cond_2a

    invoke-interface {v5}, Ljava/util/Iterator;->next()Ljava/lang/Object;

    move-result-object v6

    check-cast v6, Landroid/content/pm/Signature;

    .line 140
    .local v6, "s":Landroid/content/pm/Signature;
    if-eqz v6, :cond_29

    invoke-virtual {v6, v4}, Landroid/content/pm/Signature;->equals(Ljava/lang/Object;)Z

    move-result v7

    if-eqz v7, :cond_29

    .line 141
    const/4 v1, 0x1

    return v1

    .line 143
    .end local v6    # "s":Landroid/content/pm/Signature;
    :cond_29
    goto :goto_13

    .line 138
    .end local v4    # "signature":Landroid/content/pm/Signature;
    :cond_2a
    add-int/lit8 v3, v3, 0x1

    goto :goto_b

    .line 146
    :cond_2d
    return v1
.end method

.method private blacklist setupSignaturesMap()V
    .registers 4

    .line 49
    iget-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v1, Landroid/content/pm/Signature;

    const-string v2, ""

    invoke-direct {v1, v2}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    .line 50
    const/4 v2, 0x1

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    .line 54
    iget-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v1, Landroid/content/pm/Signature;

    const-string v2, ""

    invoke-direct {v1, v2}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    .line 55
    const/4 v2, 0x2

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    .line 59
    iget-object v0, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mSignaturesMap:Landroid/util/SparseArray;

    new-instance v1, Landroid/content/pm/Signature;

    const-string v2, ""

    invoke-direct {v1, v2}, Landroid/content/pm/Signature;-><init>(Ljava/lang/String;)V

    .line 60
    const/4 v2, 0x4

    invoke-virtual {v0, v2, v1}, Landroid/util/SparseArray;->put(ILjava/lang/Object;)V

    .line 63
    return-void
.end method

.method private blacklist setupWhiteList()V
    .registers 1

    .line 70
    return-void
.end method


# virtual methods
.method public blacklist isAllowedToUseOrder(Landroid/content/Context;Ljava/lang/String;)Z
    .registers 11
    .param p1, "context"    # Landroid/content/Context;
    .param p2, "packageName"    # Ljava/lang/String;

    .line 73
    const-string v0, "SemExecutableWhitelist"

    const/4 v1, 0x0

    .line 76
    .local v1, "signatureMatch":Z
    :try_start_3
    invoke-virtual {p1}, Landroid/content/Context;->getPackageManager()Landroid/content/pm/PackageManager;

    move-result-object v2

    .line 77
    .local v2, "packageManager":Landroid/content/pm/PackageManager;
    const/16 v3, 0x40

    invoke-virtual {v2, p2, v3}, Landroid/content/pm/PackageManager;->getPackageInfo(Ljava/lang/String;I)Landroid/content/pm/PackageInfo;

    move-result-object v3

    .line 79
    .local v3, "pkgInfo":Landroid/content/pm/PackageInfo;
    const/4 v4, 0x0

    if-nez v3, :cond_1a

    .line 80
    sget-boolean v5, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz v5, :cond_19

    const-string v5, "isAllowedToUse : pkgInfo is null"

    invoke-static {v0, v5}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 81
    :cond_19
    return v4

    .line 84
    :cond_1a
    iget-object v5, v3, Landroid/content/pm/PackageInfo;->applicationInfo:Landroid/content/pm/ApplicationInfo;

    iget v5, v5, Landroid/content/pm/ApplicationInfo;->flags:I

    and-int/lit16 v5, v5, 0x81

    const/4 v6, 0x1

    if-eqz v5, :cond_24

    .line 86
    return v6

    .line 96
    :cond_24
    iget-object v5, v3, Landroid/content/pm/PackageInfo;->signatures:[Landroid/content/pm/Signature;

    .line 97
    .local v5, "signatures":[Landroid/content/pm/Signature;
    invoke-direct {p0, v5}, Lcom/samsung/android/app/SemExecutableWhitelist;->hasSamsungKey([Landroid/content/pm/Signature;)Z

    move-result v7

    if-eqz v7, :cond_2d

    .line 99
    return v6

    .line 102
    :cond_2d
    iget-object v6, p0, Lcom/samsung/android/app/SemExecutableWhitelist;->mWhiteList:Ljava/util/HashMap;

    invoke-virtual {v6, p2}, Ljava/util/HashMap;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v6

    check-cast v6, Ljava/lang/Integer;

    .line 103
    .local v6, "type":Ljava/lang/Integer;
    if-nez v6, :cond_41

    .line 104
    sget-boolean v7, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz v7, :cond_40

    const-string v7, "isAllowedToUse : executable manager white lists does not include this App"

    invoke-static {v0, v7}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 105
    :cond_40
    return v4

    .line 107
    :cond_41
    invoke-virtual {v6}, Ljava/lang/Integer;->intValue()I

    move-result v4

    invoke-direct {p0, v5, v4}, Lcom/samsung/android/app/SemExecutableWhitelist;->isMatchedSignature([Landroid/content/pm/Signature;I)Z

    move-result v4

    move v1, v4

    .line 108
    if-nez v1, :cond_55

    .line 109
    sget-boolean v4, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz v4, :cond_55

    const-string v4, "isAllowedToUse : executable manager white lists don\'t match this App"

    invoke-static {v0, v4}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I
    :try_end_55
    .catch Landroid/content/pm/PackageManager$NameNotFoundException; {:try_start_3 .. :try_end_55} :catch_56

    .line 114
    .end local v2    # "packageManager":Landroid/content/pm/PackageManager;
    .end local v3    # "pkgInfo":Landroid/content/pm/PackageInfo;
    .end local v5    # "signatures":[Landroid/content/pm/Signature;
    .end local v6    # "type":Ljava/lang/Integer;
    :cond_55
    goto :goto_72

    .line 111
    :catch_56
    move-exception v2

    .line 112
    .local v2, "e":Landroid/content/pm/PackageManager$NameNotFoundException;
    sget-boolean v3, Lcom/samsung/android/app/SemExecutableWhitelist;->DEBUG:Z

    if-eqz v3, :cond_71

    new-instance v3, Ljava/lang/StringBuilder;

    invoke-direct {v3}, Ljava/lang/StringBuilder;-><init>()V

    const-string v4, "isNotAllowedToUse : "

    invoke-virtual {v3, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3, v2}, Ljava/lang/StringBuilder;->append(Ljava/lang/Object;)Ljava/lang/StringBuilder;

    move-result-object v3

    invoke-virtual {v3}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v3

    invoke-static {v0, v3}, Lcom/samsung/android/util/SemLog;->d(Ljava/lang/String;Ljava/lang/String;)I

    .line 113
    :cond_71
    const/4 v1, 0x0

    .line 116
    .end local v2    # "e":Landroid/content/pm/PackageManager$NameNotFoundException;
    :goto_72
    return v1
.end method
