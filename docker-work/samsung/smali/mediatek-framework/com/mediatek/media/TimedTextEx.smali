.class public Lcom/mediatek/media/TimedTextEx;
.super Ljava/lang/Object;
.source "TimedTextEx.java"


# static fields
.field private static final blacklist CLASS_NAME:Ljava/lang/String; = "android.media.TimedText"

.field private static final blacklist FD_FIELD_NAME:Ljava/lang/String; = "mBitMapFd"

.field private static final blacklist HEIGHT_FIELD_NAME:Ljava/lang/String; = "mBitMapHeight"

.field private static final blacklist TAG:Ljava/lang/String; = "TimedTextEx"

.field private static final blacklist TEXT_FIELD_NAME:Ljava/lang/String; = "mTextByteChars"

.field private static final blacklist WIDTH_FIELD_NAME:Ljava/lang/String; = "mBitMapWidth"

.field private static blacklist fdField:Ljava/lang/reflect/Field;

.field private static blacklist heightField:Ljava/lang/reflect/Field;

.field private static blacklist textField:Ljava/lang/reflect/Field;

.field private static blacklist widthField:Ljava/lang/reflect/Field;


# direct methods
.method static constructor blacklist <clinit>()V
    .registers 4

    .line 50
    const-string v0, "@M_TimedTextEx"

    :try_start_2
    const-string v1, "android.media.TimedText"

    invoke-static {v1}, Ljava/lang/Class;->forName(Ljava/lang/String;)Ljava/lang/Class;

    move-result-object v1

    .line 52
    .local v1, "cls":Ljava/lang/Class;
    const-string v2, "mTextByteChars"

    invoke-virtual {v1, v2}, Ljava/lang/Class;->getDeclaredField(Ljava/lang/String;)Ljava/lang/reflect/Field;

    move-result-object v2

    sput-object v2, Lcom/mediatek/media/TimedTextEx;->textField:Ljava/lang/reflect/Field;

    .line 53
    const-string v2, "mBitMapWidth"

    invoke-virtual {v1, v2}, Ljava/lang/Class;->getDeclaredField(Ljava/lang/String;)Ljava/lang/reflect/Field;

    move-result-object v2

    sput-object v2, Lcom/mediatek/media/TimedTextEx;->widthField:Ljava/lang/reflect/Field;

    .line 54
    const-string v2, "mBitMapHeight"

    invoke-virtual {v1, v2}, Ljava/lang/Class;->getDeclaredField(Ljava/lang/String;)Ljava/lang/reflect/Field;

    move-result-object v2

    sput-object v2, Lcom/mediatek/media/TimedTextEx;->heightField:Ljava/lang/reflect/Field;

    .line 55
    const-string v2, "mBitMapFd"

    invoke-virtual {v1, v2}, Ljava/lang/Class;->getDeclaredField(Ljava/lang/String;)Ljava/lang/reflect/Field;

    move-result-object v2

    sput-object v2, Lcom/mediatek/media/TimedTextEx;->fdField:Ljava/lang/reflect/Field;

    .line 57
    sget-object v2, Lcom/mediatek/media/TimedTextEx;->textField:Ljava/lang/reflect/Field;

    const/4 v3, 0x1

    invoke-virtual {v2, v3}, Ljava/lang/reflect/Field;->setAccessible(Z)V

    .line 58
    sget-object v2, Lcom/mediatek/media/TimedTextEx;->widthField:Ljava/lang/reflect/Field;

    invoke-virtual {v2, v3}, Ljava/lang/reflect/Field;->setAccessible(Z)V

    .line 59
    sget-object v2, Lcom/mediatek/media/TimedTextEx;->heightField:Ljava/lang/reflect/Field;

    invoke-virtual {v2, v3}, Ljava/lang/reflect/Field;->setAccessible(Z)V

    .line 60
    sget-object v2, Lcom/mediatek/media/TimedTextEx;->fdField:Ljava/lang/reflect/Field;

    invoke-virtual {v2, v3}, Ljava/lang/reflect/Field;->setAccessible(Z)V
    :try_end_3d
    .catch Ljava/lang/NoSuchFieldException; {:try_start_2 .. :try_end_3d} :catch_45
    .catch Ljava/lang/ClassNotFoundException; {:try_start_2 .. :try_end_3d} :catch_3e

    .end local v1    # "cls":Ljava/lang/Class;
    goto :goto_4b

    .line 64
    :catch_3e
    move-exception v1

    .line 65
    .local v1, "e":Ljava/lang/ClassNotFoundException;
    const-string v2, "ClassNotFoundException: android.media.TimedText"

    invoke-static {v0, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    goto :goto_4c

    .line 62
    .end local v1    # "e":Ljava/lang/ClassNotFoundException;
    :catch_45
    move-exception v1

    .line 63
    .local v1, "e":Ljava/lang/NoSuchFieldException;
    const-string v2, "NoSuchFieldException"

    invoke-static {v0, v2}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;)I

    .line 66
    .end local v1    # "e":Ljava/lang/NoSuchFieldException;
    :goto_4b
    nop

    .line 67
    :goto_4c
    return-void
.end method

.method public constructor blacklist <init>()V
    .registers 1

    .line 34
    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method public static blacklist getBitmapFd(Landroid/media/TimedText;)I
    .registers 2
    .param p0, "timedText"    # Landroid/media/TimedText;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/IllegalArgumentException;,
            Ljava/lang/IllegalAccessException;
        }
    .end annotation

    .line 122
    sget-object v0, Lcom/mediatek/media/TimedTextEx;->fdField:Ljava/lang/reflect/Field;

    invoke-virtual {v0, p0}, Ljava/lang/reflect/Field;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Integer;

    invoke-virtual {v0}, Ljava/lang/Integer;->intValue()I

    move-result v0

    return v0
.end method

.method public static blacklist getBitmapHeight(Landroid/media/TimedText;)I
    .registers 2
    .param p0, "timedText"    # Landroid/media/TimedText;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/IllegalArgumentException;,
            Ljava/lang/IllegalAccessException;
        }
    .end annotation

    .line 108
    sget-object v0, Lcom/mediatek/media/TimedTextEx;->heightField:Ljava/lang/reflect/Field;

    invoke-virtual {v0, p0}, Ljava/lang/reflect/Field;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Integer;

    invoke-virtual {v0}, Ljava/lang/Integer;->intValue()I

    move-result v0

    return v0
.end method

.method public static blacklist getBitmapWidth(Landroid/media/TimedText;)I
    .registers 2
    .param p0, "timedText"    # Landroid/media/TimedText;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/IllegalArgumentException;,
            Ljava/lang/IllegalAccessException;
        }
    .end annotation

    .line 94
    sget-object v0, Lcom/mediatek/media/TimedTextEx;->widthField:Ljava/lang/reflect/Field;

    invoke-virtual {v0, p0}, Ljava/lang/reflect/Field;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Ljava/lang/Integer;

    invoke-virtual {v0}, Ljava/lang/Integer;->intValue()I

    move-result v0

    return v0
.end method

.method public static blacklist getTextByteChars(Landroid/media/TimedText;)[B
    .registers 2
    .param p0, "timedText"    # Landroid/media/TimedText;
    .annotation system Ldalvik/annotation/Throws;
        value = {
            Ljava/lang/IllegalArgumentException;,
            Ljava/lang/IllegalAccessException;
        }
    .end annotation

    .line 80
    sget-object v0, Lcom/mediatek/media/TimedTextEx;->textField:Ljava/lang/reflect/Field;

    invoke-virtual {v0, p0}, Ljava/lang/reflect/Field;->get(Ljava/lang/Object;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, [B

    return-object v0
.end method
